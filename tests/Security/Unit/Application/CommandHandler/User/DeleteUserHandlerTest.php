...
use Marvin\Security\Application\CommandHandler\User\DeleteUserHandler;
...
final class DeleteUserHandlerTest extends TestCase
{
    ...
    public function testThrowsExceptionWhenDeletingLastAdmin(): void
    {
        $userId = new UserId();
        $command = new DeleteUser($userId);
        ...
        $this->lastUserAdminVerifier
            ...
            ->method('verify')
            ->with($user)
            ->willThrowException(new LastUserAdmin('Cannot delete the last admin user'));
