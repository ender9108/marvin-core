...
final readonly class ExecuteDeviceActionProcessor implements ProcessorInterface
{
    ...
    /**
     * @param ExecuteDeviceActionDto $data
     * @throws Exception
     */
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ReadDeviceResource
    {
        Assert::isInstanceOf($data, ExecuteDeviceActionDto::class);
        Assert::keyExists($uriVariables, 'id');

        $model = $this->syncCommandBus->handle(new ExecuteDeviceAction(
            DeviceId::fromString($uriVariables['id']),
            Capability::from($data->capability),
            CapabilityAction::from($data->action),
            $data->parameters,
        ));
